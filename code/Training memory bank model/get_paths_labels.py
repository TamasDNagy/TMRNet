...
root_dir2 = '/mnt/Storage/acmit/Autolaparo/Autolaparo_Task1/'
img_dir2 = os.path.join(root_dir2, 'frame')
...
phase_dict = {}
phase_dict_key =["Preparation",
               "Dividing Ligament\nand Peritoneum",
               "Dividing Uterine\nVessels and Ligament",
...
for j in range(len(phase_file_names2)):
    ...
    for phase_line in phase_file:
        ...
        if int(phase_split[0]) % downsample_rate == 0:
            info_each = []
            img_file_each_path = os.path.join(img_dir_paths2[j], str(int(phase_split[0])) + '.jpg')
            info_each.append(img_file_each_path)
            info_each.append(int(phase_split[1]) - 1)
